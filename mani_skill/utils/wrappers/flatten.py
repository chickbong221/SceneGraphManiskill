...
class FlattenRGBDObservationWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation: dict):
        ...
        if len(rgb_images) > 0:
            rgb_images = [torch.as_tensor(x) if isinstance(x, np.ndarray) else x for x in rgb_images]
            rgb_images = torch.concat(rgb_images, axis=-1)
        if len(depth_images) > 0:
            depth_images = [torch.as_tensor(x) if isinstance(x, np.ndarray) else x for x in depth_images]
